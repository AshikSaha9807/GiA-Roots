...
namespace BioNic {
    ...
    private:
        boost::shared_ptr<BnDataWrapper<void> > result;
    protected:
        virtual void * doTransform (BnDataWrapper<void> * data, set<string> & newTags) = 0;
        void setResult (boost::shared_ptr<BnDataWrapper<void> > result);
    public:
        GiaTransform(const string & name);
        virtual ~GiaTransform();
        boost::shared_ptr<BnDataWrapper<void> > transform (boost::shared_ptr<BnDataWrapper<void> > data);
        virtual void feed (boost::shared_ptr<BnDataWrapper<void> > data);
        
        virtual boost::shared_ptr<BnDataWrapper<void> > getSampleResult ();
