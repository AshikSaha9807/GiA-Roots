...
#define BN_DECLARE extern "C"
    ...
    class GiaOperator {
    public:
        GiaOperator (string _name) : logger(0), name(_name) { }
        virtual ~GiaOperator() { }
        string getName ();
    ...
    public:
        ...
        void setRequiredInput(boost::shared_ptr<BnDataWrapper<void> > data);
        boost::shared_ptr<BnDataWrapper<void> > getProducedOutput(const string & typeName);
    ...
    protected:
        ...
        void setProducedOutput (boost::shared_ptr<BnDataWrapper<void> > output, void * data);
    ...
    private:
        void populateKeys(const map<string, boost::shared_ptr<BnDataWrapper<void> > > & source, list<string> & keys);
         ...
         */
        map<string, boost::shared_ptr<BnDataWrapper<void> > > requiredInputs;
        map<string, boost::shared_ptr<BnDataWrapper<void> > > producedOutputs;
        map<string, boost::shared_ptr<BnDataWrapper<void> > > mockedInputs;
        map<string, boost::shared_ptr<BnDataWrapper<void> > > mockedOutputs;
    ...
    void BnAlgorithm::requiresInput() {
        ...
        if (requiredInputs.find( temp ) == requiredInputs.end()) {
            requiredInputs.insert(make_pair( temp, boost::shared_ptr<BnDataWrapper<void> >() ));
            mockedInputs.insert(make_pair( temp, boost::shared_ptr<BnDataWrapper<void> > (P::createNewInstanceStatic()) ));
        }
    }
    ...
    void BnAlgorithm::producesOutput() {
        ...
        if (mockedOutputs.find( temp ) == mockedOutputs.end())
            mockedOutputs.insert(make_pair( temp, boost::shared_ptr<BnDataWrapper<void> > (P::createNewInstanceStatic()) ));
    }
    
    template<class PDataWrapper, typename P>
    void BnAlgorithm::setProducedOutput (P data) {
        setProducedOutput(boost::shared_ptr<BnDataWrapper<void> >( PDataWrapper::descriptor() ), new P(data));
    }

    template<class PDataWrapper, typename P>
    void BnAlgorithm::setProducedOutput (P* data) {
        setProducedOutput(boost::shared_ptr<BnDataWrapper<void> >( PDataWrapper::descriptor() ), data);
    }
