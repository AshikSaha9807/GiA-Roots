...
#define BN_TYPE_OF(x) x::getTypeNameStatic()
    ...
    typedef list< boost::shared_ptr<BnDataWrapper<void> > > BnDataWrapperList;
...
//        BnDataWrapper(const BnDataWrapper &copy);
    ...
    public:
        ...
        void readSerializationInfo(boost::shared_ptr<GiaDataHeader> info, const map<string,string> & config);
        void writeSerializationInfo(boost::shared_ptr<GiaDataHeader> info, const map<string,string> & config, bool headerOnly = false);
...
//        BnDataWrapper(const BnDataWrapper &copy);
    ...
    void BasicDataWrapper<T>::deserialize(const string & value, const map<string,string> & config) {
        ...
        DataType temp;
        ss >> temp;
        this->setData(new DataType(temp));            
    }
}
